...
def validate(device, batch_size, model,sia_network, criterion,
    ...
    with torch.no_grad():
        ...
        for i in range(total_step):
            ...
            # Forward pass
            out, out_feature_fc,out_feature_xyz = model(
                input_shape_mouth,
...
class ConvNet3D_one(nn.Module):
    ...
    def forward(self,
        ...
        # print('out.shape = ', out.shape)
        out = self.fc_sum(out)
        return out, out_fetures_fc, features_xyz
...
def train():
    ...
    for epoch in range(no_epochs):
        ...
        for i in range(total_step):
            ...
            model.train()
            sia_network.train()
            outputs, out_feature_fc, features_xyz = model(
                          input_shape_mouth,
                          input_shape_eyeLeft, input_shape_eyeRight,
            ...
            out_feature_fc = np.reshape(out_feature_fc,(out_feature_fc.shape[0],
                                                        out_feature_fc.shape[1],
                                                        out_feature_fc.shape[2],
            ...
            for m in range(out_feature_fc.shape[0]):
                ...
                for n in range(normal_num):
                    ...
                    # model.eval()
                    outputs_two, out_feature_fc_two, features_xyz_two = model(
            ...
            Loss = 0.9 * (avg_loss_sia+ np.mean(loss_sia_loc_arr)) + 0.1 * loss
