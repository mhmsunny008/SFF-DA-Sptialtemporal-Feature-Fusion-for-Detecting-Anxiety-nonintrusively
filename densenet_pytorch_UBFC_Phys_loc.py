    ...
    with torch.no_grad():
        ...
        for i in range(total_step):
            if i < total_step - 1:
                ...
                input_shape_mouth = torch.from_numpy(data_set_mouthRIO_test[i * batch_size:(i + 1) * batch_size]).to(
                    device).float()
                input_shape_eyeLeft = torch.from_numpy(
                    data_set_eyeLeftRIO_test[i * batch_size:(i + 1) * batch_size]).to(device).float()
                input_shape_eyeRight = torch.from_numpy(
                    data_set_eyeRightRIO_test[i * batch_size:(i + 1) * batch_size]).to(device).float()
                input_shape_xyz = torch.from_numpy(data_set_xyz_test[i * batch_size:(i + 1) * batch_size]).to(
                    device).float()
...
class DenseNet(nn.Module):
    ...
    def feature_extract_iPPG(self, x):
        ...
        # features_arr = np.array(features_arr)
        print('features_arr.shape = ', features_arr.shape)
        features_arr = self.fc_iPPG(torch.from_numpy(features_arr).to(device))
        return features_arr
    ...
    def forward(self,
        ...
        # features_iPPG = self.feature_extract_LSTM(input_shape_iPPG)
...
def train():
    ...
    # ********************* 4. 头部位姿：数据处理************************************************************* #
    # 4. 头部位姿：数据处理
    data_set_loc_x_T1 = np.load(
        r'/home/ps/lab-data/MoHaiMiao/2021-05-anxiety_screen-paper2/2021-11-09-dataset_UBFC_Phys/dataset_UBFC_Phys_T1/loc_T1_56_6090_468/'
        + 'loc_x_T1_56_6090_468.npy')
    data_set_loc_y_T1 = np.load(
        r'/home/ps/lab-data/MoHaiMiao/2021-05-anxiety_screen-paper2/2021-11-09-dataset_UBFC_Phys/dataset_UBFC_Phys_T1/loc_T1_56_6090_468/'
        + 'loc_y_T1_56_6090_468.npy')
    data_set_loc_z_T1 = np.load(
        r'/home/ps/lab-data/MoHaiMiao/2021-05-anxiety_screen-paper2/2021-11-09-dataset_UBFC_Phys/dataset_UBFC_Phys_T1/loc_T1_56_6090_468/'
    ...
    loc_arr_T1 = np.reshape(loc_arr_T1,
                            (data_set_loc_x_T1.shape[0], data_set_loc_x_T1.shape[1], data_set_loc_x_T1.shape[2], 3))
    ...
    save_name = '_' + 'epochs' + str(
        no_epochs) + '_' + 'mouth' + '_' + 'eyeLeft' + '_' + 'eyeRight' + '_' + 'location'
    # + '_' + 'mouth' \
