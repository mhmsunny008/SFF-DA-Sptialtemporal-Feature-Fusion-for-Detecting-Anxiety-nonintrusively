    ...
    with torch.no_grad():
        ...
        for i in range(total_step):
            if i < total_step - 1:
                ...
                input_shape_iPPG = torch.from_numpy(data_set_iPPG_test[i * batch_size:(i + 1) * batch_size]).to(
                    device).float()
        ...
        for i in range(total_step):
            ...
            if (i + 1) % 34 == 0:
                ...
                if acc > temp_acc:
                    ...
                    np.savetxt('./data_result_resnet_40_40pixel/resnet_y_pred_probability' + save_name + '.txt',
                               y_pred_probability)
