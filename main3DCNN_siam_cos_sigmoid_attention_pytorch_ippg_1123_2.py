...
def siamese_input_reshape(data): # input size [96, 1, 252, 10]
    # feature_num = 5 # 需要修改   96*4*1*63
    # print('data.shape = ', data.shape)
    # 1280=256*5=128*10=64*2*10=32*2*2*10=16*8*10=8*16*10
    data = torch.reshape(data, (8,  # 96
                                1,  # 1
                                8,  #
                                10))
    return data
            ...
            with torch.no_grad():
                ...
                for m in range(out1.shape[0]):
                    ...
                    for n in range(anxiety_num):
                        ...
                        input1_siamese = siamese_input_reshape(out1_fetures[m])
                        input2_siamese = siamese_input_reshape(out2_fetures)
                        sim= sia_network(input1_siamese, input2_siamese)
                        # loss_sim
                        ...
                        sim_arr.append(similarity.data.cpu().numpy())
                # 转换成数组
                ...
                Loss = 0.1 * (np.mean(sim_arr)) + 0.9 * loss
                ...
                if (i + 1) % 32 == 0:
                    ...
                    if acc > temp_acc:
                        ...
                        np.savetxt(
                            './data_result_ours_attention_40_40pixel_1123/3DCNN_sigmoid_attention_data_result_2' + save_name + '.txt',
                            data_result)
                        np.savetxt(
                            './data_result_ours_attention_40_40pixel_1123/3DCNN_sigmoid_attention_y_test_2' + save_name + '.txt',
                            y_true)
                        np.savetxt(
                            './data_result_ours_attention_40_40pixel_1123/3DCNN_sigmoid_attention_y_pred_probability_2' + save_name + '.txt',
                            y_pred_probability)
                        np.savetxt(
                            './data_result_ours_attention_40_40pixel_1123/3DCNN_sigmoid_attention_sigmoid_y_pred_2' + save_name + '.txt',
                            y_pred)
