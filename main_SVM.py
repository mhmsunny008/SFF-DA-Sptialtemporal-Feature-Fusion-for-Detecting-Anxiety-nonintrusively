...
def train():
    ...
    ippg_forehead = data_set_ippg_forehead.reshape((data_set_ippg_forehead.shape[0], data_set_ippg_forehead.shape[1] * data_set_ippg_forehead.shape[2]))
    ...
    ippg_nose  = data_set_ippg_nose.reshape((data_set_ippg_nose.shape[0], data_set_ippg_nose.shape[1] * data_set_ippg_nose.shape[2]))
    ...
    # svm_model = svm.SVC(C=1, kernel='rbf', gamma="auto", decision_function_shape='ovr')
    # svm_model.fit(x_train, y_labels_train.ravel())
    # preds = svm_model.predict(x_train)
    # preds_test = svm_model.predict(x_test)

    # ------- SVM --------#
    print('--- SVM model ---')
    # svm_model = SVC(probability=True, random_state=1, tol=1e-6)  # SVM模型 tol:svm停止训练的误差精度，float类型，可选参数，默认为1e^-3
    ...
    # # save result
    save_name = '_' + 'epochs' + str(no_epochs) + '_' + 'mouth'+ '_' + 'eyeLeft' + '_' + 'eyeRight'+ '_' + 'iPPG'
    # + '_' + 'mouth' \
