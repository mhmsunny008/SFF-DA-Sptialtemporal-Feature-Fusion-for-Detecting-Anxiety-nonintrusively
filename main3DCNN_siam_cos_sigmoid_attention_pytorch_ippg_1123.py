...
def validate(device, batch_size, model,sia_network, criterion,
    ...
    model.eval()
    sia_network.eval()
    gen_signals_val = data_set_loc_test
    ...
    with torch.no_grad():
        ...
        for i in range(total_step):
            if i < total_step - 1:
                ...
                input_shape_loc = torch.from_numpy(data_set_loc_test[i * batch_size:(i + 1) * batch_size]).to(device).float()
                ...
                input_shape_loc = torch.from_numpy(data_set_loc_test[i * batch_size:-1]).to(device).float()
            ...
            # Forward pass
            out, out_mouth, out_eyeLeft, out_eyeRight, out_loc, out_iPPG = model(
                input_shape_mouth,
...
class ConvNet3D_one(nn.Module):
    def __init__(self,
        ...
        self.fc_sum = nn.Sequential(
                                    nn.Linear(64 * num_features, self.num_classes),
    ...
    def forward(self,
        ...
        # print('out.shape = ', out.shape)
        out = self.fc_sum(out)
        return out,  features_mouth, features_eyeLeft, features_eyeRight,features_loc,features_iPPG,
...
def train():
    ...
    for epoch in range(no_epochs):
        ...
        for i in range(total_step):
            ...
            sia_network.train()
            with torch.no_grad():
                out1, out1_mouth, out1_eyeLeft, out1_eyeRight, out1_loc, out1_iPPG = model(
                              input_shape_mouth,
                              input_shape_eyeLeft, input_shape_eyeRight,
                ...
                for m in range(out1.shape[0]):
                    ...
                    for n in range(anxiety_num):
                        ...
                        # 经过特征提取网络进行特征提取
                        out2, out2_mouth, out2_eyeLeft, out2_eyeRight, out2_loc, out2_iPPG= model(
                        ...
                        # 计算相似度
                        sim_mouth = similar_cal(out1_mouth[m], out2_mouth)
                        sim_eyeLeft = similar_cal(out1_eyeLeft[m], out2_eyeLeft)
                        sim_eyeRight = similar_cal(out1_eyeRight[m], out2_eyeRight)
                        sim_location = similar_cal(out1_loc[m], out2_loc)
                ...
                Loss = 0.1*(np.mean(loss_mouth_arr) + np.mean(loss_eyeLeft_arr)
                ...
                if (i + 1) % 32 == 0:
                    ...
                    if acc > temp_acc:
                        ...
                        np.savetxt(
                            './data_result_ours_attention_40_40pixel_1123/3DCNN_sigmoid_attention_data_result' + save_name + '.txt',
                            data_result)
                        np.savetxt(
                            './data_result_ours_attention_40_40pixel_1123/3DCNN_sigmoid_attention_y_test' + save_name + '.txt',
                            y_true)
                        np.savetxt(
                            './data_result_ours_attention_40_40pixel_1123/3DCNN_sigmoid_attention_y_pred_probability' + save_name + '.txt',
                            y_pred_probability)
                        np.savetxt(
                            './data_result_ours_attention_40_40pixel_1123/3DCNN_sigmoid_attention_sigmoid_y_pred' + save_name + '.txt',
                            y_pred)
