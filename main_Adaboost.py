...
def train():
    ...
    model_Adaboost = AdaBoostClassifier(DecisionTreeClassifier(max_depth=2, min_samples_split=20, min_samples_leaf=5),
                             algorithm="SAMME",
                             n_estimators=1, learning_rate=0.5)  # 选择了SAMME算法，最多300个弱分类器，步长0.5；n_estimators:基分类器提升（循环）次数，默认是50次
    test_predict_label = model_Adaboost.fit(x_train, y_train).decision_function(x_test)
    ...
    # # save result
    save_name = '_' + 'epochs' + str(no_epochs) + '_' + 'mouth' + '_' + 'eyeLeft' + '_' + 'eyeRight' + '_' + 'location' # 额头提取iPPG
    # + '_' + 'mouth' \
    ...
    np.savetxt('./data_result_Adaboost_40p_40pixel/SVM_fpr' + save_name + '.txt', fpr)
    np.savetxt('./data_result_Adaboost_40p_40pixel/SVM_tpr' + save_name + '.txt', tpr)
    np.savetxt('./data_result_Adaboost_40p_40pixel/SVM_data_result-' + save_name + '.txt', data_result)
    np.savetxt('./data_result_Adaboost_40p_40pixel/SVM_y_test' + save_name + '.txt', y_true)
    np.savetxt('./data_result_Adaboost_40p_40pixel/SVM_y_pred_probability' + save_name + '.txt',
               y_pred_probability)
    np.savetxt('./data_result_Adaboost_40p_40pixel/SVM_y_pred' + save_name + '.txt', y_pred)
